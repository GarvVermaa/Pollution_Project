...
function processRecords(raw) {
 try {
   ...
   return raw.map(r => {
     if (!r || typeof r !== 'object') throw new Error("Invalid record format");
     
     return {
       station_id: r.station_id,
       station_no: r.station_no,
       station_name: r.station_name?.replace(/^[A-Z]{2}\d+_/, "").trim() || "Unknown",
       state: r.territory_name || "Unknown",
       latitude: parseFloat(r.station_latitude) || 0,
       longitude: parseFloat(r.station_longitude) || 0,
       timestamp: r.timestamp,
       parameter: r.stationparameter_longname || "Unknown",
       value: r.ts_value,
